...
这套数据工程不是“把 Spider 转成 JSONL”这么简单。它要同时解决五个问题：统一中英文任务、保证 Gold 可执行、控制 schema 交叉风险、构造高信息密度的 SFT 轨迹，并为 SFT 与 RL 记录清晰的任务来源和评测口径。
...
核心约束是三个主划分文件之间没有数据库重叠。原因是 Text-to-SQL 模型很容易记住 schema 名、列名和常见 join 路径；如果只随机拆问题，即便问题不同，模型仍可能在验证集见过同一个数据库结构，指标会虚高。

`final_eval` 不用于超参数选择或 checkpoint 选择。训练期只使用 dev 派生出的评测集。

这里必须区分“划分设计”和“历史实验事实”：划分文件本身是 DB 互斥的，但正式 SFT 混合数据经历了多阶段迁移，现有 checkpoint 指标不能包装成严格的未见 schema 泛化结果。严格结论需要重新审计训练样本来源，并用完全未参与数据构造和选模的数据库评测。
...
| 类型 | 主要来源 | Oracle / 真实决策 | 是否必含失败反馈 | 学习目标 |
|---|---|---|---|---|
| `sql_core` | Gold SQL + Schema + Gold Result | Oracle | 否 | 直接 SQL 语义 |
| `schema_only` | Gold SQL 自动拼接最短工具轨迹 | Oracle | 否 | Schema 与执行闭环 |
| `agent_trace` | Gold SQL 自动拼接完整工具流程 | Oracle | 否 | 标准多轮流程 |
| `teacher_agent` | 历史 Teacher 真实环境 rollout | 真实决策 | 否 | observation 驱动的工具选择 |
| `repair_real` | Repair 来源 rollout 与轨迹内修复 | 混合 | **否，只有 142/251 明确包含** | 失败后的下一动作与迭代修正 |
| `protocol_anchor` | 从已有轨迹截取的关键短片段 | 继承来源 | 否 | JSON 协议和终止边界 |
| `teacher_agent_real_v3` | 新批次困难任务 Teacher rollout | 真实决策 | 否 | 高难度多表行为 |

完整 `messages` 示例、构造流程和样本 ID 见[《SFT 训练与数据构造》](../技术设计/SFT训练与数据构造.md)。
...
先校正一个容易误解的地方：`repair_real` 是历史数据来源标签，不代表 251 条全部是标准修复轨迹。按正式 `messages` 审计，其中 142 条明确出现失败 observation 并继续行动，33 条没有显式失败但执行了多条 SQL，另外 76 条其实是普通成功轨迹。因此，下面描述的是其中“严格 Repair 样本”要教授的能力。

严格 Repair 样本主要覆盖模型执行 SQL 后暴露的问题，例如：
...
### 7.1 Repair loop 到底在哪里发生

需要把三个阶段分开：

| 阶段 | 是否真的执行 SQLite | 实际发生的事 |
|---|---|---|
| 离线数据构造 | 是 | 首次 rollout 失败后，verifier 生成结构化反馈；Teacher 从包含失败 SQL 和反馈的 `seed_messages` 继续，最终成功才成为候选数据 |
| SFT 训练 | 否 | 完整修复轨迹已经写死；训练通过 teacher forcing，让模型学习“看到失败 observation 后，下一个 Assistant 动作应该是什么” |
| 推理 / RL rollout | 是 | runtime 执行当前动作，把新的 `tool_result` 追加进上下文，模型基于真实反馈在线重试 |

因此，SFT 中的 repair loop 不是“训练时 Agent 自己执行失败后现场重跑”，而是**离线记录好的失败—反馈—修正序列**。模型学到这个条件行为后，推理或 RL 时才可能在真实环境中复现它。

还要区分两种反馈来源：

- 语法错误、缺表、缺列等执行错误可以由 SQLite 当场返回；
- `wrong_result` 表示 SQL 能执行但结果与 Gold Result 不一致，只能由数据构造阶段或最终评测的 verifier 判断，普通 runtime 不能仅看预测结果就知道语义错了。

V1 到 V2 的 Repair、Teacher 和协议变化见[《SFT 数据演进》](../项目演进/SFT数据演进.md)。
...
- 是否够用由固定 Agent rollout 指标判断，不由行数自证；
...
这个项目在数据设计上使用 DB 级划分：train/dev/final_eval 文件按数据库隔离。但对已经完成的历史训练，不能只凭 split manifest 推断训练集与评测集严格隔离；必须继续审计每条 SFT/Teacher 数据的来源。还要注意两条工程边界：
...
> 我们先把 Spider 和 CSpider 归一化成统一 Task，按 `(db_id, normalized_gold_sql)` 合并中英文重复任务，再执行每条 Gold SQL 缓存完整结果，过滤不可执行、过大和超时样本。划分文件按完整数据库拆分 train/dev/final_eval；历史 SFT 又经历过多阶段混合，所以我会把“划分设计”和“当前结果的证据边界”分开讲，不把它夸大为严格未见 schema 泛化。最终 SFT 有 5,817 条，由 SQL core、协议锚点、多轮轨迹、真实 repair 和 331 条 verifier 成功的 Teacher 轨迹组成。
